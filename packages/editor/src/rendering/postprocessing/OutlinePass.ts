...
class OutlinePass extends Pass {
     ...
     * @type {Array<Object3D>}
     */
    public selectedObjects: Node3D[];

    /**
     * The visible edge color.
     *
     */
    public visibleEdgeColor = new Color(1, 1, 1);

    /**
     * The hidden edge color.
     *
     */
    public hiddenEdgeColor = new Color(0.1, 0.04, 0.02);
     ...
     *
     */
    public usePatternTexture = false;
     ...
     *
     */
    public downSampleRatio = 2;

    /**
     * The pulse period.
     *
     */
    public pulsePeriod = 0;

    public _visibilityCache = new Map<Node3D, boolean>();
    public _selectionCache = new Set<Node3D>();
    ...
    public copyUniforms: { [key: string]: any };
    ...
    public _fsQuad = new FullScreenQuad(/* null */ new MeshBasicMaterial());

    public tempPulseColor1 = new Color();
    public tempPulseColor2 = new Color();
     ...
     * @param {Array<Node3D>} [selectedObjects] - The selected 3D objects that should receive an outline.
     *
     */
    constructor(
        resolution: Vector2,
        ...
        function replaceDepthToViewZ(string: string, camera: Camera): string {

            const type = 'isPerspectiveCamera' in camera ? 'perspective' : 'orthographic';
    ...
    public render(
        ...
        if (this.selectedObjects.length > 0) {
            ...
            if (isPerspectiveCamera(this.renderCamera) || isOrthographicCamera(this.renderCamera)) {

                this.prepareMaskMaterial.uniforms['cameraNearFar'].value.set(this.renderCamera.near, this.renderCamera.far);

            } else {

                console.warn('OutlinePass: render - Unsupported camera type')
    ...
    private _updateSelectionCache() {
        ...
        function gatherSelectedMeshesCallBack(object: Node3D) {

            // if ('isMesh' in object) cache.add(object);
    ...
    private _changeVisibilityOfNonSelectedObjects(bVisible: boolean) {
        ...
        function VisibilityChangeCallBack(object: Node3D) {

            if ('isPoints' in object || 'isLine' in object || 'isLine2' in object) {
            ...
            } else if ('isMesh' in object || 'isSprite' in object) {
        ...
        return new ShaderMaterial({
            ...
            fragmentShader:
				...
				void main() {
					...
					// gl_FragColor = vec4(0.0, depthTest, 1.0, 1.0);   // TODO:
    ...
    private _getSeparableBlurMaterial(maxRadius: number) {
