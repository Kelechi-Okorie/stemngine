...
/**
 * Class for representing the armatures in `js`. The skeleton
...
class Skeleton {
	 ...
	 * @default null
	 */
	public previousBoneMatrices: Float32Array | null = null;
	...
	public pose(): void {
		...
		for (let i = 0, il = this.bones.length; i < il; i++) {
			...
			if (bone) {
				...
				if (bone.parent && 'isBone' in bone.parent) {
	 ...
	 * @return {Skeleton} A reference of this instance.
	 */
	public fromJSON(json: any, bones: Record<string, Bone>): Skeleton {
	...
	public toJSON(): any {
		...
		for (let i = 0, l = bones.length; i < l; i++) {
			...
			data.boneInverses.push(Array.from(boneInverse.toArray()));
...
function isSkeleton(skeleton: any): skeleton is Skeleton {
	return (skeleton as Skeleton).isSkeleton === true;
}

export { Skeleton, isSkeleton };
