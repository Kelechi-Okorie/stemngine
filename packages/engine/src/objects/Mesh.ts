...
import { Raycaster, RaycasterIntersection, RaycastableObject } from '../core/Raycaster.js';
...
export class Mesh extends Node3D {
  ...
  public _computeIntersections(
    raycaster: Raycaster,
    intersects: RaycasterIntersection[],
    rayLocalSpace: Ray
  ) {

    let intersection: RaycasterIntersection | null;
        ...
        for (let i = 0, il = groups.length; i < il; i++) {
          ...
          for (let j = start, jl = end; j < jl; j += 3) {
            ...
            if (intersection) {
              ...
              intersection.face!.materialIndex = group.materialIndex ?? 0;
...
function checkGeometryIntersection(
  ...
  raycaster: Raycaster,
  ray: Ray,
  uv: BufferAttribute | InterleavedBufferAttribute | null,
  uv1: BufferAttribute | InterleavedBufferAttribute | null,
  normal: BufferAttribute | InterleavedBufferAttribute | null,
  a: number,
  b: number,
  c: number
): RaycasterIntersection | null {
