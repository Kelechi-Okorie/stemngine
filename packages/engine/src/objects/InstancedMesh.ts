...
import { Raycaster, RaycasterIntersection } from '../core/Raycaster.js';

const _instanceLocalMatrix = /*@__PURE__*/ new Matrix4();
const _instanceWorldMatrix = /*@__PURE__*/ new Matrix4();

const _instanceIntersects: RaycasterIntersection[] = [];
...
export class InstancedMesh extends Mesh {
  ...
  // TODO: type well
	public copy( source: any, recursive: any ) {
    ...
    const instanceColor = this.instanceColor;

    if (!instanceColor) {
      throw new Error('InstancedMesh.getColorAt(): instanceColor is null.');
    }
	 ...
	 */
	public getMatrixAt( index: number, matrix: Matrix4 ) {
	 ...
	 */
	public getMorphAt( index: number, object: Mesh ) {

  if (!this.morphTexture) {
    throw new Error('InstancedMesh has no morphTexture');
  }

  if (!object.morphTargetInfluences) {
    throw new Error('Target mesh has no morph targets');
  }
	 ...
	 */
	public setMorphAt( index: number, object: Mesh ) {
  ...
  if (objectInfluences === undefined) {
    throw new Error(
      'InstancedMesh.setMorphAt(): object has no morphTargetInfluences.'
    );
  }
