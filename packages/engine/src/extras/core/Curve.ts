...
import { Vector2, isVector2 } from '../../math/Vector2';
 ...
 */
  ...
  public getLengths(divisions: number = this.arcLengthDivisions): number[] {
    ...
    for (let p = 1; p <= divisions; p++) {
      ...
      if (current instanceof Vector3 && last instanceof Vector3) {

        sum += current.distanceTo(last);

      } else if (current instanceof Vector2 && last instanceof Vector2) {
   ...
   */
  public getUtoTmapping(u: number, distance: number | null = null): number {
  ...
  public getTangent(t: number, optionalTarget?: T): T {
    ...
    if (pt1 instanceof Vector3 && pt2 instanceof Vector3 && tangent instanceof Vector3) {

      tangent.copy(pt2).sub(pt1).normalize();

    } else if (pt1 instanceof Vector2 && pt2 instanceof Vector2 && tangent instanceof Vector2) {

      tangent.copy(pt2).sub(pt1).normalize();

    } else {

      console.warn('Mismatched vector types');
   ...
   */
  public computeFrenetFrames(segments: number, closed = false) {
